...
int main()
{
	...
	*/
	...
	//////////////Compression ratio////////////////////
	cout << "���Y��" << ((Mx * Nx) * (log(256) / log(2))) / (((Mx * Nx) / (Ax * Bx)) * (log(64) / log(2))) << endl;
	cout << "���" << (1 - ((((Mx * Nx) / (Ax * Bx)) * (log(64) / log(2))) / ((Mx * Nx) * (log(256) / log(2))))) * 100 << "%��ƶq" << endl;
	cout << ((Mx * Nx) / (Ax * Bx)) * (log(64) / log(2)) << "��ƶq" << endl;

	if (!bmpWrite(imageVec, "gray_clone.bmp"))
	{
	...
	if (!bmpWrite(imageVecNew, "gray_2x2_cpp.bmp"))
	{
		cout << "Write image error!!" << endl;
		system("PAUSE");
		return -1;
